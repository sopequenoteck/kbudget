...
- **KKS-382 — Reimporter un releve ne bloque plus sur les lignes deja importees** : 240 lignes sur 294 etaient `DUPLICATE` bloquantes, toutes a raison. Une ligne deja importee passe `SKIPPED` + `skipReason: ALREADY_IMPORTED` — **aucune valeur d'enum nouvelle**, un client ancien voit une ligne ignoree. Reconnaissance par une **empreinte** de la ligne de releve (date comptable, montant, sens, libelle brut) posee sur la transaction, donc insensible au renommage ; a defaut (import anterieur), libelle nettoye identique, l'empreinte etant alors posee a la confirmation. Chaque transaction ne sert qu'une fois : deux lignes identiques legitimes restent deux operations.
- **KKS-324 — Les clients traduisent le code d'erreur, plus le message serveur** : `message` devient un champ de diagnostic en anglais, seul `error` est contractuel. Catalogue Angular des 27 codes, discrimination par code cote Flutter. **Clients livres avant le serveur** : l'inverse aurait montre de l'anglais entre deux livraisons. Un client web 6.5.x encore en cache affiche ces messages en anglais jusqu'a son rechargement.
- **KKS-357 — Les codes d'erreur servis aux clients, verrouilles** : Angular et Flutter branchent sur le corps d'erreur, pas sur le `message`. **Le ticket affirmait a tort que rien ne les couvrait** — `GlobalExceptionHandlerTest` verifie les 24 codes du handler avec leur statut, et les quatre emis hors du handler ont chacun leur test. Le trou etait ailleurs : `details[].code` derive du nom de l'annotation Bean Validation, et le test existant construisait son `FieldError` a la main, verifiant la mise en majuscules mais jamais le nom d'annotation dont elle part. Remplacer `@Size` par `@Length` ne cassait rien. `ValidationErrorCodeContractTest` part des DTOs et du validateur reels — **verifie en cassant le code expres**, il echoue sur `expected: "SIZE" but was: "LENGTH"`. `ExceptionHandlerInventoryTest` inventorie les 24 handlers declares : les tests de codes sont des listes tenues a la main, rien n'obligeait un handler nouveau a y entrer. **Sa comparaison est stricte dans les deux sens, a l'inverse d'`ApiContractIT`** — un champ de reponse ajoute est absorbe par un client ancien, un code d'erreur ajoute est un mot que personne n'a vu. Ne pas aligner les deux. Volet springdoc ecarte : 100 operations a annoter ajouteraient plus de mille lignes repetitives a un snapshot qui en compte 1050.
- **KKS-358 — Une PR de fork ne declenchait aucun test API ni Flutter** : le depot est public depuis le 2026-09-05, mais `ci-api.yml` n'avait qu'un seul job et il tournait sur le runner self-hosted, inaccessible a un fork. Une PR externe touchant `api/` n'affichait donc rien ; cote Flutter, seuls les en-tetes de licence etaient verifies. `api-tests` et `flutter-tests` ajoutes sur `ubuntu-latest` — 620 tests unitaires et 34 d'integration cote API, 917 cote Flutter, mesures sur la PR qui les introduit : elle modifie les trois workflows, qui figurent chacun dans leur propre `paths`, donc elle s'auto-teste. **`api-tests` ne tourne pas dans un container** : deux IT demarrent un PostgreSQL via Testcontainers et ont besoin d'un Docker joignable, ce que le gate de release savait deja. Le runner self-hosted ne gagne aucun job — il a le socket Docker. Sonar y reste, le serveur vivant sur un reseau prive : une PR de fork n'a pas de Quality Gate, ce qui est desormais ecrit dans `CONTRIBUTING.md` au lieu d'etre decouvert. KKS-312 avait fait ce travail pour Angular seulement.
