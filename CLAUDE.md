...
- **KKS-395 — Cle stable des categories systeme** : prealable 1/3 de KKS-394. Colonne `system_key` (`SUBSCRIPTION`, `DEBT`, `TRANSFER`, `ADJUSTMENT`, V39, backfill depuis le nom francais), enum `SystemCategoryKey`. L'API retrouve les categories systeme **par cle, jamais par nom** (`CategoryService.findSystemCategory`). `CategoryResponse.systemKey` et `categorySystemKey` sur les DTO a plat (budgets, regles et lignes d'import). Angular : pipe impur `categoryName` et `categoryDisplayName` en TS ; tri et creation restent sur `nom` (ordre de l'API). Le `nom` stocke reste francais : reporte a KKS-396 avec les autres textes ecrits par l'API.
- **KKS-380 — Bascule : anglais par defaut et selecteur de langue** : derniere etape de KKS-325. Preference `null` = suit le navigateur (`navigator.languages`, sinon anglais), jamais ecrite sans choix explicite. Derniere langue appliquee memorisee dans `localStorage` (`kbudget.language`) pour le demarrage et les ecrans non connectes ; la preference serveur fait foi des qu'elle arrive. `LanguageService` distingue langue **demandee** et **appliquee** : `activeLanguage` ne change qu'apres chargement du catalogue, et une resolution obsolete n'ecrase pas une demande plus recente. Selecteur dans Reglages > Apparence (`SelectPicker`, noms natifs `LANGUAGE_NATIVE_NAMES`). API : `DELETE /users/me/preferences/language` (retour a « Automatique »), `PUT` ne pouvant pas remettre `null`. Tests : `provideTranslocoTesting()` fixe navigateur `fr` et stockage memoire via `BROWSER_LANGUAGES` / `LANGUAGE_STORAGE`, d'ou une suite toujours en francais.
